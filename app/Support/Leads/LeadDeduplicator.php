...
class LeadDeduplicator
{
     ...
     * сделки для одного номера). Тонкая обёртка над withLock — см. её докблок про
     * гарантии (ровно один вызов колбэка, fallback без блокировки при её отказе).
     *
     * @template T
    ...
    public static function withPhoneLock(int $accountId, ?string $phone, callable $callback)
    {
        ...
        return self::withLock('phone:'.$accountId.':'.$phone, $callback);
    }

    /**
     * Выполнить $callback под именованной блокировкой (сериализация конкурентных
     * событий с одним и тем же ключом — номер телефона, диалог чата и т.п.), чтобы
     * гонка (два вебхука/поллинга одного и того же события) не создавала дубликаты.
     * Если блокировку не удалось взять за отведённое время, либо сам кэш-стор
     * недоступен (диск/права и т.п.) — выполняем без блокировки (лучше без дедупа,
     * чем потерять лид/сообщение).
     *
    ...
    {
        $lock = Cache::lock('lead-lock:'.$key, 15);
