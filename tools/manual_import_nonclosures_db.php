...
if (!$xlsxPath) {
    ...
    fwrite(STDERR, "  php tools/manual_import_nonclosures_db.php /absolute/path/to/file.xlsx [account_id] [user_id] [--truncate] [--dry-run]\n");
    exit(1);
}
...
echo "Режим полной перезаливки: ".($truncate ? 'да' : 'нет')."\n\n";
...
DB::transaction(function () use ($service, $xlsxPath, $accountId, $userId, $truncate, &$result): void {
    if ($truncate) {
        NonClosure::query()->where('account_id', $accountId)->delete();
    }
...
echo " - строк в таблице сейчас: ".NonClosure::query()->where('account_id', $accountId)->count()."\n";
